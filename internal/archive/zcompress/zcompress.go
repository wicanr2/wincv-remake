...
func Decode(data []byte) ([]byte, error) {
	if !IsZ(data) {
		return nil, fmt.Errorf("不是 compress 檔(magic 不對)")
	}
	maxbits := int(data[2] & 0x1F)
	blockMode := data[2]&0x80 != 0
	if maxbits < initBits || maxbits > 16 {
		return nil, fmt.Errorf("maxbits = %d 超出範圍", maxbits)
	}
	...
	for {
		...
		if oldCode < 0 {
			if code >= 256 {
				return out, fmt.Errorf("第一個碼是 %d,應該是單一位元組", code)
			}
			finChar = byte(code)
		...
		if code >= freeEnt {
			...
			if code > freeEnt {
				return out, fmt.Errorf("字碼 %d 超出字典(%d)", code, freeEnt)
			}
