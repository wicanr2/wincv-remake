...
**檔案清單那 16 列的屬性差是 0** —— 三級驗收的第二級(屬性等價)在清單區成立。
剩下的 12 格屬性差全在路徑列,原因是路徑字串本身不一樣
(`C:\wincv\` vs `/src/original/app/`),欄位的起點跟著移位。

版面差與像素差同樣多半是**資料不同**而不是畫錯:

- 檔案的修改時間不一樣(原版那份是 Wine 裡的複本)。
- Wine 把長檔名顯示成 8.3 大寫短名(`BIG5~X5T TXT`),重製版顯示原名。
- 第 92 欄是原版的 Win32 捲軸控制項,不在格點上,結構上比不出等價。

還沒實作而確實造成差異的:第二列狀態列(原版有兩列)、左側磁碟欄、捲軸。
