...
`V` 那一路是真的把頁面畫成點陣圖:路徑、填色與描邊、影像、以及嵌入
字型的字形外框。表格、圖表、公式、簽名那些「意義在位置上」的東西,
抽成文字就沒了,只有畫出來才看得到。

目前的限制:

- **嵌入字型三種格式都解外框**(TrueType/OpenType、CFF、Type1)。三種都解
  不開時才用系統字型照同樣的位置與字級畫,並在畫面上說明換過哪些字型。
- **奇偶填法(`f*`)照非零繞組畫**,光柵器不支援前者。同方向的內圈會被
  填實而不是留成洞。
- **非矩形的裁剪取外接矩形**,會多顯示一點,但不會少顯示。
- **漸層(shading)與圖樣(pattern)還沒畫**,那些區域會留白。
