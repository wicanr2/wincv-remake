...
支援繁體中文(原生)、簡體中文、English、日本語。決定用哪一個的優先序:

1. `-lang zh-Hant | zh-Hans | en | ja` 旗標
2. session 記住的(選過就記,見 `session.State.Lang`)
3. 系統語系(`LC_ALL` / `LC_MESSAGES` / `LANG`;Android 由 Java 那一側
   把 `Locale.getDefault().toLanguageTag()` 傳進來 —— Android 沒有 `LANG`)
4. 都沒有就是繁體中文

`F9` → 設定 → 語言 可以隨時換,下一幀就生效(選單標籤、狀態列、觸控列
都是每次繪製才組出來的,不必重開)。

實作在 `internal/i18n`:**key 就是繁體中文原文**,不另外發明訊息 ID。
讀原始碼的人看到 `T("檔案")` 就知道畫面上會出現什麼;漏翻的字串會原樣
顯示成中文,而不是 `menu.file.label` 這種東西。

`F1` 使用說明另外有各語言一份(`internal/app/help.*.md`),缺哪一份就
退回繁體中文那份。

**中文以外的字形靠後備字型。** 倚天是 Big5 字庫,沒有日文假名(Big5 的
假名落在符號補充區 C6A1–C8FE,而那一區的索引還沒解出來,一律當缺字),
也沒有簡體字。所以日文與簡體介面的字是系統的 TrueType 或完整版內嵌的
Noto 畫的,與點陣字混排時字形不同 —— 那是字庫的邊界,不是設定問題。
...
| `V` | 把這一頁**整頁畫出來**,進看圖模式;`+` / `-` 放大、`PgUp` / `PgDn` 換頁(見「看圖」一節);`Esc` 退回同一頁的文字 |
| `Backspace` | 上一頁 |
...
| `+` / `-` | 放大 / 縮小 | — | 重製版新增,見下 |
| `PgUp` / `PgDn` | 上一張 / 下一張圖;**PDF 整頁模式下是換頁** | — | 重製版新增 |
| `1` | 回到原尺寸(1:1) | — | 重製版新增 |
| `F` | 「縮放到視窗」與「指定倍率」之間切換 | — | 重製版新增 |

放大倍率走一組階梯:0.25 / 0.33 / 0.5 / 0.67 / 1 / 1.5 / 2 / 3 / 4 / 6 / 8。
不用固定的乘數是為了讓整數倍落在階梯上 —— 點陣圖放大到整數倍時每個來源
像素才會攤成同樣大小的方塊,與內容的放大倍率是同一件事。

**PDF 的整頁圖放大時會用更高的解析度重畫**,不是把 150 dpi 的點陣圖拉大。
差別在放大之後看不看得到更多東西:表格的細線、小字的註腳,點陣放大只會
讓它們變成更大的糊塊。一頁重畫後超過 4000 萬個像素就不重畫(那時一張
RGBA 要 160 MB),退回點陣放大 —— 畫面會糊,但還在動。

實作上這是 `imgview.Model.Rerender` 這個可選的 hook:有它就重畫,沒有就
點陣放大。看圖模式本身不知道自己畫的是 PDF 還是 PNG。

**整頁模式下 `PgUp` / `PgDn`(與 `Enter` / `BackSpace`)是換頁。** 看圖模式
原本的「上一張 / 下一張」走的是目錄裡的圖檔清單(`imageNeighbours`),
而 PDF 的整頁圖不在那份清單上 —— 游標停在那個 `.pdf` 上,按下去會跳到
目錄裡不相干的另一張圖,或什麼都不做。換頁時放大倍率跟著走(放大到 2 倍
在讀表格的人翻頁之後還在讀表格),而且 `bv` 的內容一起換掉,所以 `Esc`
退回瀏覽模式看到的是**同一頁**的文字。
