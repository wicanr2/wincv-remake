...
- **物件層交不出來的照片**:DCTDecode(JPEG)配 ICCBased 色彩空間時,
  pdfcpu 的影像抽取回空,那張照片就整塊不見。這是目前真實檔案上最大的
  一個缺口(arXiv 論文那一頁的圖全是這種)。串流本身就是一份完整的 JPEG,
  用 `image/jpeg` 直接解得開 —— 補這條退路是下一件事。
...
| arXiv 論文內頁(取樣函式的漸層)| 0.7043 | 照片沒畫出來(見下),漸層本身逐點與 poppler 差 1/255 以內 |
| 投影片(放射漸層)| 0.9800 | — |
