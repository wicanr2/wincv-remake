...
**影像**:第一條路是交給 pdfcpu(它把幾十種色彩空間、位元深度、Decode
陣列、遮罩的組合收斂成一張 png 或 jpg)。它交不出來的時候自己解:
只經過 DCTDecode 的串流本身就是一份完整的 JPEG;其餘的濾鏡物件層會解掉,
剩下的是原始取樣值,那一段與內嵌影像走同一個解碼器。影像自己的 `/SMask`
也在這條路上套上去(尺寸可以與影像不同,各自縮放)。

**透明度**:常數透明度(`ca` / `CA`)、亮度與 alpha 兩種**軟遮罩**
(ExtGState 的 `/SMask`)、**透明群組**(表單物件的 `/Group`)。後兩者都要
先畫到一張暫時畫布上再整批合成:軟遮罩是把另一段內容畫出來、拿它的亮度
當每個像素的透明度(漸層條、羽化的陰影、淡出的色塊都是這樣做的);
透明群組是「整組算完再套外層的透明度」,不然群組裡自己設的透明度會蓋掉
外層的,該淡淡一片的東西會變成實色。
...
| `seac.pdf` 重音字合成 | poppler | — | — | 0.9997 |
| `softmask.pdf` 透明度 | poppler | 0.0002 | 0.0400 | 0.9999 |
| `image.pdf` 影像退路 | poppler | 0.0007 | 0.0417 | 0.9997 |
...
|---|---|---|
| 報價單(漸層圖樣)| 0.9536 | — |
| arXiv 論文首頁(軸向漸層 + 接合函式)| 0.9942 | — |
| arXiv 論文次頁(取樣函式的漸層)| 0.9586 | — |
| arXiv 論文內頁(照片 + 取樣函式的漸層)| 0.9782 | — |
| 投影片(放射漸層)| 0.9800 | — |
| 行銷報告(亮度軟遮罩 + 透明群組)| 0.9949 | — |
| 自組的 seac 頁(嵌 Bitstream Charter)| 0.9994 | — |

這六頁一開始分別是 0.77 / 0.99 / 0.52 / 0.71 / 0.24 / 0.98。
