...
現在的 `pdfdoc` 用 `rsc.io/pdf` 取文字,缺口是:

- **CID / `ToUnicode`**:中文 PDF 多半用 CID 字型,字碼不是 Unicode。
  沒有走 `ToUnicode` CMap 就會抽出一串對不上的字,而且看起來像亂碼不像失敗。
- **分欄**:同一條掃描線上有兩欄的內容,現在會接成一列。
- **大綱**:PDF 的 `/Outlines` 是現成的目錄,現在的「目錄」只是一排頁碼。
- **加密**:標準安全處理常式(空密碼的 RC4 / AES)的檔案現在整份打不開。
- **字寬**:沒有嵌入字型時算不出前進量,詞間空白還原不了。
