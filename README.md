...
原版讀不到的東西也接上了：gopher 站台、網頁、EPUB 電子書、PDF，走的是同一個
畫面與同一組按鍵。做法是把它們一律壓成「一段一段的文字、一個一個的連結、
幾張圖」——那正好是這個字元格點畫得出來的東西，也正好是 1999 年那套介面
本來就在做的事。
...
**不打包進任何產物**，由使用者自備。逐項的著作權與出處在 [NOTICE](NOTICE)。
...
| 散布方式 | 共享軟體，作者對外發行 | BSD 2-Clause，產物不含原版素材 | 字型、字典資料與倚天字庫都是第三方版權物，由使用者自備 |
...
| PDF | `.pdf` 按 `Enter` 直接開第一頁，頁末有上下頁。取出的是**文字與圖片，不是版面**——分欄的文件會左右欄交錯，圖片放在頁末 |
