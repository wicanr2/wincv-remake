...
| 2026-09-02 | `tools/release.sh` 不再清空 `dist-all/`,只刪自己要重出的那幾個檔| 完整版也放在同一個目錄,而它不是 release.sh 產的。整個刪掉的話,兩支腳本的執行順序就變成一個沒有人會記得的隱藏相依 —— 反過來跑會安靜地少掉四個檔,而目錄看起來仍然是滿的。「發布物對得上同一個 commit」這個保證改由「先刪自己要重出的 + 逐一確認比 HEAD 新」兩件事撐著 |
